...
export default {
  ...
  getNotes: async (request: Request, response: Response) => {
    ...
    try {
      ...
      response
        .status(400)
        .send({ message:  (error as Error).message || 'Something went wrong while fetching note data' })
    }
  },
  ...
  getCategories: async (request: Request, response: Response) => {
    ...
    try {
      ...
      response
        .status(400)
        .send({ message:  (error as Error).message || 'Something went wrong while fetching category data' })
    }
  },
}
